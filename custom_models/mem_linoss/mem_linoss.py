...
class MemLinOSS(nn.Module):
    ...
    def _forward_short_conv(
        ...
        # ShortConvolution backend probing may trigger graph breaks under torch.compile.
        q, conv_state_q = self.q_conv1d(
            x=self.q_proj(hidden_states).contiguous(),
            cache=conv_state_q,
            output_final_state=use_cache,
            cu_seqlens=cu_seqlens
        )
        k, conv_state_k = self.k_conv1d(
            x=self.k_proj(hidden_states).contiguous(),
            cache=conv_state_k,
            output_final_state=use_cache,
            cu_seqlens=cu_seqlens
        )
        v, conv_state_v = self.v_conv1d(
            x=self.v_proj(hidden_states).contiguous(),
            cache=conv_state_v,
            output_final_state=use_cache,
            cu_seqlens=cu_seqlens
        )
        return q, k, v, conv_state_q, conv_state_k, conv_state_v
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        if self.use_short_conv:
            ...
            q, k, v, conv_state_q, conv_state_k, conv_state_v = self._forward_short_conv(
        ...
        if self.use_beta:
            beta = self.b_proj(hidden_states).sigmoid() * self.base_beta
        else:
            beta = torch.ones_like(q[..., 0]) * self.base_beta
        ...
        if mode == 'fused_recurrent':
            o, recurrent_state = self.linoss.recurrent_forward(
                q=q,
                k=k,
                v=v,
                beta=beta,
            ...
            o, recurrent_state = self.linoss.chunk_forward(
                q=q,
                k=k,
                v=v,
                beta=beta,
