...
from .mem_linoss import MemLinOSS
...
class MemLinOSSBlock(GradientCheckpointingLayer):

    def __init__(self, config: MemLinOSSConfig, layer_idx: int):
        super().__init__()
...
class MemLinOSSPreTrainedModel(PreTrainedModel):

    config_class = MemLinOSSConfig
    base_model_prefix = 'model'
    supports_gradient_checkpointing = True
    _no_split_modules = ['MemLinOSSBlock']
    ...
    def _init_weights(
        ...
        if getattr(module, '_keep_default_init', False):
...
class MemLinOSSModel(MemLinOSSPreTrainedModel):

    def __init__(self, config: MemLinOSSConfig):
...
class MemLinOSSForCausalLM(MemLinOSSPreTrainedModel, GenerationMixin):

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MemLinOSSModel(config)
        self.vocab_size = config.vocab_size
